...
// ─── Fallback: generate plan locally when API unavailable
...
// ─── Apply AI-generated plan (from API response with food name strings)
...
function _generateSmartAlerts(){
 try{
  var sup=parseInt(localStorage.getItem('dieton_alerts_suppressed_until')||'0');
...
function _clearAllNotifs(){
 notifs=[];
 // Suppress auto-generated alerts for 24h
 try{ localStorage.setItem('dieton_alerts_suppressed_until', Date.now() + 86400000); }catch(ex){}
 updateNotifBadge();
 if(typeof DB!=='undefined')DB.save();
 goP('notif', document.getElementById('ni-notif'));
 showToast('Todas as notificações limpas','s');
}
// ════════════════════════════════════════════════════════════
...
// Run reminders check on login and every hour
...
function _filterChatList(q){
 var items = document.querySelectorAll('#chat-list .card');
 items.forEach(function(el){
  var name = el.querySelector('span') ? el.querySelector('span').textContent.toLowerCase() : '';
  el.style.display = (!q||name.includes(q.toLowerCase())) ? '' : 'none';
 });
}
...
openChat = function(patId){
 var p = patId ? pats.find(function(x){return x.id===patId;}) : selPat||pats[0];
 if(p && p.msgs) p.msgs.forEach(function(m){if(m.role==='pac') m.read=true;});
 _updateChatBadge();
 _origOpenChat(patId);
};
...
// ════════════════════════════════════════════════════════════
// BACKEND SYNC — save key data to Railway backend
// ════════════════════════════════════════════════════════════
async function apiSyncAll(){
 if(typeof apiUpdatePatient!=='function')return;
 var p=selPat;if(!p)return;
 try{ await apiUpdatePatient(p); }catch(e){}
}
...
// Patch DB.save to also trigger auto-sync
...
function _buildWeekView(){
 ...
 if(window._agView==='week') return _buildWeekView();
...
// ─── Auto-alert for follow-up appointments
 ...
 load: function(){
  ...
  try{
   ...
   if(data.pats&&data.pats.length){
    pats=data.pats;
    // Migrate: ensure all patients have required arrays (backward compat)
    pats.forEach(function(p){
     if(!p.appointments)p.appointments=[];
     if(!p.suplementos)p.suplementos=[];
     if(!p.orientacoes)p.orientacoes=[];
     if(!p.diary)p.diary=[];
     if(!p.plans)p.plans=[];
...
var _tourSteps = [
  { el: null, icon: '🎉', title: 'Bem-vindo ao DietOn!', body: 'Este tour rápido vai te mostrar as principais funcionalidades. Use os botões abaixo para navegar — leva menos de 2 minutos!', pos: 'center' },
  { el: 'ni-pat', icon: '👥', title: 'Cadastrar Pacientes', body: 'Acesse a lista de pacientes. Use o botão <strong>+ Novo Paciente</strong> para cadastrar com anamnese completa, exames e histórico clínico.', pos: 'right' },
  { el: 'ni-notif', icon: '🔔', title: 'Notificações', body: 'Alertas automáticos dos seus pacientes — IMC crítico, vitamina D baixa, glicemia alterada e muito mais. O ponto vermelho indica alertas pendentes.', pos: 'right' },
  { el: 'ni-rec', icon: '📋', title: 'Recordatório 24h', body: 'Registre o que o paciente comeu nas últimas 24 horas, refeição por refeição.', pos: 'right' },
  { el: 'ni-ai', icon: '🤖', title: 'IA Nutricional', body: 'Gere planos alimentares completos com inteligência artificial em segundos. Escolha entre 8 objetivos clínicos.', pos: 'right' },
  { el: 'ni-supl', icon: '💊', title: 'Suplementação', body: 'Prescreva suplementos para cada paciente e acompanhe a adesão.', pos: 'right' },
  { el: 'ni-fin', icon: '💰', title: 'Financeiro', body: 'Registre cobranças e marque pagamentos. Todos os dados ficam salvos no banco!', pos: 'right' },
  { el: null, icon: '✅', title: 'Tudo pronto!', body: 'Explore o sistema à vontade. O botão <strong>Tutorial</strong> no topo sempre abre este guia novamente. Comece cadastrando seu primeiro paciente!', pos: 'center' }
];
...
function rBusca(){
 return '<div style="max-width:720px">'
  ...
  +'<svg style="position:absolute;left:14px;top:50%;transform:translateY(-50%);color:var(--n4)" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg>'
  +'<input class="inp" id="global-q" placeholder="Buscar por nome, objetivo, diagnóstico..." oninput="globalSearch()" style="padding-left:44px;font-size:14px;height:48px;border-radius:14px"/></div>'
  // ── Filtros avançados
  +'<div class="card" style="margin-bottom:16px">'
  +'<div class="ch" style="cursor:pointer" onclick="_toggleAdvFilters()">'
  +'<span class="ct">🎛️ Filtros avançados</span>'
  +'<span id="adv-filter-toggle" style="font-size:12px;color:var(--g4)">▼ Expandir</span></div>'
  +'<div id="adv-filters" style="display:none">'
  +'<div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:10px">'
  // Objetivo
  +'<div><label class="lbl">Objetivo</label>'
  +'<select class="sel" id="f-obj" onchange="globalSearch()" style="width:100%">'
  +'<option value="">Todos</option>'
  +'<option>Emagrecimento</option><option>Hipertrofia</option>'
  +'<option>Manutenção</option><option>Saúde geral</option><option>Condição clínica</option>'
  +'</select></div>'
  // Status
  +'<div><label class="lbl">Status</label>'
  +'<select class="sel" id="f-status" onchange="globalSearch()" style="width:100%">'
  +'<option value="">Todos</option>'
  +'<option value="tg">Em dia ✅</option>'
  +'<option value="ty">Atenção ⚠️</option>'
  +'<option value="tr">Alerta 🔴</option>'
  +'</select></div>'
  // IMC
  +'<div><label class="lbl">Classificação IMC</label>'
  +'<select class="sel" id="f-imc" onchange="globalSearch()" style="width:100%">'
  +'<option value="">Todos</option>'
  +'<option value="baixo">Abaixo do peso (&lt;18.5)</option>'
  ...
  +'</select></div>'
  // Última consulta
  +'<div><label class="lbl">Última consulta</label>'
  +'<select class="sel" id="f-appt" onchange="globalSearch()" style="width:100%">'
  +'<option value="">Qualquer</option>'
  +'<option value="7">Últimos 7 dias</option>'
  +'<option value="30">Últimos 30 dias</option>'
  +'<option value="60">Últimos 60 dias</option>'
  +'<option value="90+">Sem consulta há 90+ dias</option>'
  +'</select></div>'
  +'</div>'
  +'<button class="btn btn-ghost btn-sm" onclick="_clearAdvFilters()" style="margin-top:10px">Limpar filtros</button>'
  +'</div></div>'
  // ── Resultados
  +'<div id="search-results"><div style="text-align:center;padding:32px;color:var(--n4);font-size:13px">Digite para buscar ou escolha um filtro acima</div></div>'
  +'</div>';
}
...
function globalSearch(){
 ...
 var fObj=(document.getElementById('f-obj')||{}).value||'';
 var fStatus=(document.getElementById('f-status')||{}).value||'';
 var fImc=(document.getElementById('f-imc')||{}).value||'';
 var fAppt=(document.getElementById('f-appt')||{}).value||'';

 var res=document.getElementById('search-results');
 if(!res)return;
 if(!q&&!fObj&&!fStatus&&!fImc&&!fAppt){
  res.innerHTML='<div style="text-align:center;padding:32px;color:var(--n4);font-size:13px">Digite para buscar ou escolha um filtro acima</div>';
  return;
 }

 var now=new Date();
 var found=pats.filter(function(p){
  var ql=q.toLowerCase();
  // Text search
  if(q){
   var hay=(p.n||'')+(p.email||'')+(p.obj||'')+(p.diag||'');
   if(!hay.toLowerCase().includes(ql))return false;
  }
  // Objetivo
  if(fObj&&(p.obj||'')!==fObj)return false;
  // Status
  if(fStatus){
   var tag=_patTag(p);
   if(tag!==fStatus)return false;
  }
  // IMC
  if(fImc&&p.w&&p.h){
   var imc=p.w/((p.h/100)*(p.h/100));
   if(fImc==='baixo'&&imc>=18.5)return false;
   if(fImc==='normal'&&(imc<18.5||imc>=25))return false;
   if(fImc==='sobrepeso'&&(imc<25||imc>=30))return false;
   if(fImc==='obesidade'&&imc<30)return false;
  }
  // Última consulta
  if(fAppt){
   var appts=(p.appts||[]).filter(function(a){return a.done;});
   var lastAppt=appts.length?new Date(appts[appts.length-1].date||appts[appts.length-1].dt):null;
   var daysSince=lastAppt?Math.round((now-lastAppt)/864e5):9999;
   if(fAppt==='7'&&daysSince>7)return false;
   if(fAppt==='30'&&daysSince>30)return false;
   if(fAppt==='60'&&daysSince>60)return false;
   if(fAppt==='90+'&&daysSince<90)return false;
  }
  return true;
 });

 if(!found.length){
  res.innerHTML='<div style="text-align:center;padding:32px;color:var(--n4);font-size:13px">Nenhum paciente encontrado</div>';
  return;
 }

 var html='<div style="font-size:11px;color:var(--n4);margin-bottom:10px">'+found.length+' resultado'+(found.length!==1?'s':'')+'</div>';
 found.forEach(function(p){
  var imc=p.w&&p.h?(p.w/((p.h/100)*(p.h/100))).toFixed(1):null;
  var tag=_patTag(p);
  var tagColors={tg:'#22c55e',ty:'#f59e0b',tr:'#ef4444'};
  var appts=(p.appts||[]).filter(function(a){return a.done;});
  var lastAppt=appts.length?new Date(appts[appts.length-1].date||appts[appts.length-1].dt):null;
  var daysSince=lastAppt?Math.round((now-lastAppt)/864e5):null;
  var initials=p.n?p.n.split(' ').slice(0,2).map(function(w){return w[0];}).join(''):'?';
  html+='<div class="card" style="display:flex;align-items:center;gap:14px;padding:12px 14px;margin-bottom:8px;cursor:pointer;transition:border-color .15s" '
   +'onclick="selPat=p;if(typeof buildMainUI===\'function\')buildMainUI();" '
   +'onmouseover="this.style.borderColor=\'var(--g4)\'" onmouseout="this.style.borderColor=\'\'">'
   +'<div class="av '+(p.av||'a1')+'" style="width:44px;height:44px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:14px;font-weight:800;flex-shrink:0">'+initials+'</div>'
   +'<div style="flex:1;min-width:0">'
   +'<div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin-bottom:3px">'
   +'<span style="font-weight:700;font-size:13px;color:var(--n9)">'+escHtml(p.n)+'</span>'
   +(tag?'<span style="width:8px;height:8px;border-radius:50%;background:'+(tagColors[tag]||'#ccc')+';flex-shrink:0"></span>':'')
   +(p.obj?'<span style="font-size:10px;background:var(--g0);color:var(--g5);padding:1px 7px;border-radius:99px">'+escHtml(p.obj)+'</span>':'')
   +'</div>'
   +'<div style="display:flex;gap:12px;flex-wrap:wrap">'
   +(imc?'<span style="font-size:11px;color:var(--n5)">IMC <strong>'+imc+'</strong></span>':'')
   +(p.w?'<span style="font-size:11px;color:var(--n5)">Peso <strong>'+p.w+'kg</strong></span>':'')
   +(daysSince!==null?'<span style="font-size:11px;color:'+(daysSince>60?'#ef4444':daysSince>30?'#f59e0b':'var(--n5)')+'">Última consulta: '+(daysSince===0?'hoje':daysSince+' dias')+'</span>':'<span style="font-size:11px;color:var(--n4)">Sem consultas</span>')
   +'</div></div>'
   +'<button class="btn btn-p btn-sm" onclick="event.stopPropagation();selPat=pats.find(function(x){return x.id==='+p.id+'});goP(\'pdash\',null)">Ver →</button>'
   +'</div>';
 });
 res.innerHTML=html;
}
...
function initPro(){
 ...
 nav.innerHTML=`
  ...
  <button class="ni" id="ni-agenda" onclick="goP('agenda',this)"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M19 4h-1V2h-2v2H8V2H6v2H5c-1.11 0-2 .9-2 2L3 20c0 1.1.89 2 2 2h14c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 16H5V10h14v10zm0-12H5V6h14v2zm-7 5h5v5h-5z"/></svg>Agenda<span class="nb nb-g" id="nb-agenda" style="display:none">0</span></button>
  <button class="ni" id="ni-chat" onclick="goP('chat',this)"><svg viewBox="0 0 24 24" fill="currentColor"><path d="M20 2H4c-1.1 0-2 .9-2 2v18l4-4h14c1.1 0 2-.9 2-2V4c0-1.1-.9-2-2-2zm0 14H6l-2 2V4h16v12z"/></svg>Mensagens<span class="nb nb-r" id="nb-chat" style="display:none">0</span></button>
...
function buildBChart(){
 ...
 for(var i=7;i>=0;i--){
  ...
  var wStart=new Date(d);wStart.setDate(d.getDate()-6);
  var wEnd=new Date(d);
  var count=0;
  pats.forEach(function(p){
   (p.appointments||[]).forEach(function(a){
    var ad=new Date(a.isoDate||a.date||'');
    if(ad>=wStart&&ad<=wEnd)count++;
   });
  });
  data.push(count);
  labels.push('S'+(8-i));
 ...
 if(!totalAppts){
  el.innerHTML='<div style="display:flex;flex-direction:column;align-items:center;justify-content:center;height:100%;padding:30px 0;color:var(--n4);font-size:12.5px;gap:8px">'
   ...
   +'<span>Nenhuma consulta registrada ainda</span></div>';return;
 }
...
function confirmDeletePat(id){
 ...
 if(p&&typeof apiDeletePatient==='function'){
  apiDeletePatient(p._backendId||p.id).catch(function(){});
 }
 DB.save();
 closeM('m-del-pat');
...
function rPresc(){
 ...
 <div>
  ...
  <div style="display:flex;gap:8px;margin:10px 0">
   <button class="btn btn-p" style="flex:1" onclick="savePlan()">💾 Salvar Plano para o Paciente</button>
   <button class="btn btn-ghost" onclick="exportPDF()">⬇ PDF</button>
   </div>
   <div class="card" style="margin-top:0">
   <div class="ch"><span class="ct">Histórico de Planos</span><button class="btn btn-ghost btn-sm" onclick="openM('m-tpl')">💾 Salvar Template</button></div>
...
function buildPlanHist(){
 ...
 var html=plans.map(function(pl,i){
  return'<div class="plan-ver'+(i===0?' active':'')+'">'
   +'<div><div class="pv-date">'+pl.date+'</div>'
   +'<div class="pv-meta">'+(pl.kcal||0)+' kcal · '+(pl.meals||pl.mealData&&pl.mealData.length||0)+' refeições</div></div>'
   +'<span class="pv-badge" style="background:#fdd0a8;color:#6b2d00">'+(pl.name||'Plano #'+(i+1))+'</span>'
   +'<div style="display:flex;gap:4px">'
   +'<button class="btn btn-ghost btn-sm" onclick="loadPlan('+i+')">Carregar</button>'
   +(i===0?'<button class="btn btn-ghost btn-sm" onclick="printPlan()" title="Imprimir plano">🖨️</button>':'')
   +(i===0&&plans.length>1?'<button class="btn btn-ghost btn-sm" onclick="openPlanCompare()" title="Comparar com anterior">⇄</button>':'')
   +'</div></div>';
 }).join('');
 return html;
}
...
// ─── Plan comparison modal — show two plans side by side
...
// ─── removeFood (alias para removeFoodItem com args diferentes)
...
// ─── saveWeekPlan — persiste plano semanal do paciente
...
// ─── calcMicro — recalcula micronutrientes ao mudar paciente
function calcMicro(){
 goP('micro',document.getElementById('ni-micro'));
}

// ─── saveRec — salva Recordatório 24h no paciente
function saveRec(){
 var p=selPat||pats[0];if(!p){showToast('Selecione um paciente','w');return;}
...
// ─── addRecItem — adiciona linha de alimento no Recordatório
...
function rWeek(){
 ...
 html+='<div style="display:flex;justify-content:flex-end;margin-bottom:14px;margin-top:12px"><button class="btn btn-p" onclick="saveWeekPlan()">💾 Salvar Plano Semanal</button></div>';
 return html;
}
// ─── EVOLUTION ───
function rEv(){
 ...
 var goalW=p.goalW||(p.goal&&p.goal.toLowerCase().includes('emagre')?Math.max(p.w-10,p.h/100*p.h/100*22):null);
 var wsWithGoal=goalW?ws.concat([goalW]):ws;
 var mn=Math.min.apply(null,wsWithGoal)-2,mx=Math.max.apply(null,wsWithGoal)+2;
 ...
 +(goalW?'<line x1="30" y1="'+(16+(mx-goalW)/(mx-mn)*(ch-28)).toFixed(1)+'" x2="'+(cw-10)+'" y2="'+(16+(mx-goalW)/(mx-mn)*(ch-28)).toFixed(1)+'" stroke="#f59e0b" stroke-width="1.5" stroke-dasharray="6,3"/><text x="'+(cw-8)+'" y="'+((16+(mx-goalW)/(mx-mn)*(ch-28))-3).toFixed(1)+'" font-size="9" fill="#f59e0b" text-anchor="end">meta '+goalW+'kg</text>':'')+circles+labels+yvals+'</svg></div>'
 +imcHtml+'<div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">'
 ...
 // ── IMC evolution chart
 var imcHist=hist.filter(function(h){return h.imc;});
 var imcHtml='';
 if(imcHist.length>1){
  var imcVals=imcHist.map(function(h){return h.imc;});
  var imcMin=Math.min.apply(null,imcVals)-1,imcMax=Math.max.apply(null,imcVals)+1;
  var icw=500,ich=60;
  var ipts=imcHist.map(function(h,i){
   var x=30+i*(icw-60)/(imcHist.length>1?imcHist.length-1:1);
   var y=8+(imcMax-h.imc)/(imcMax-imcMin)*(ich-16);
   return{x:x.toFixed(1),y:y.toFixed(1),v:h.imc,d:h.data};
  });
  var iline=ipts.map(function(p,i){return(i===0?'M':'L')+p.x+','+p.y;}).join(' ');
  var normalY=(8+(imcMax-24.9)/(imcMax-imcMin)*(ich-16)).toFixed(1);
  var imcColor=p.bmi<18.5?'#3b82f6':p.bmi<=24.9?'#22c55e':p.bmi<=29.9?'#f59e0b':'#ef4444';
  imcHtml='<div class="card" style="margin-bottom:14px"><div class="ch" style="margin-bottom:8px">'
   +'<span class="ct">Evolução do IMC</span>'
   +'<span class="cs" style="color:'+imcColor+';font-weight:700">Atual: '+p.bmi.toFixed(1)+'</span></div>'
   +'<svg viewBox="0 0 '+icw+' '+(ich+20)+'" width="100%" height="'+(ich+20)+'">'
   +'<line x1="30" y1="'+normalY+'" x2="'+(icw-10)+'" y2="'+normalY+'" stroke="#22c55e" stroke-width="1" stroke-dasharray="4,3"/>'
   +'<text x="34" y="'+(parseFloat(normalY)-3)+'" font-size="8" fill="#22c55e">Normal (≤24.9)</text>'
   +'<path d="'+iline+'" fill="none" stroke="'+imcColor+'" stroke-width="2" stroke-linecap="round"/>'
   +ipts.map(function(pt){return'<circle cx="'+pt.x+'" cy="'+pt.y+'" r="3" fill="'+imcColor+'"><title>'+pt.v.toFixed(1)+' — '+pt.d+'</title></circle>';}).join('')
   +ipts.map(function(pt,i){return(i===ipts.length-1||i===0)?'<text x="'+pt.x+'" y="'+(parseFloat(pt.y)-7)+'" text-anchor="middle" font-size="8" fill="'+imcColor+'">'+pt.v.toFixed(1)+'</text>':'';}).join('')
   +'</svg></div>';
 }
}
...
function rMicro(){
 ...
 +calcTMB(selPat||pats[0]);
}

// ─── 24H RECALL ───
function rRec(){
 var p=selPat||pats[0];
 if(!p) return '<div class="alert alert-b"><span>Selecione um paciente primeiro.</span></div>';

 // Compare with prescribed plan
 var prescPlan=null;
 if(p.plans&&p.plans.length>0&&p.plans[0].mealData){
  prescPlan=p.plans[0].mealData.map(function(m){
   var k=m.items.reduce(function(s,it){
    var f=FOOD_DB.find(function(x){return x.id===it.fid;});
    return s+Math.round((f?f.k:100)*(it.qty||100)/100);
   },0);
   return{n:m.name,k:k};
  });
 }
 var tkPresc=prescPlan?prescPlan.reduce(function(s,m){return s+m.k;},0):0;

 // Previous recordatorios
 var prevRecs=(p.recordatorios||[]).slice(0,3);

 // Meal slots
 var mealSlots=['Café da manhã','Lanche da manhã','Almoço','Lanche da tarde','Jantar','Ceia'];

 var html='';

 // Header status
 html+='<div class="alert alert-b" style="margin-bottom:14px">'
  +'<span>📋 Registre o que o paciente consumiu nas últimas 24h para comparar com o plano prescrito'
  +(tkPresc?(' — meta: <strong>'+tkPresc+' kcal</strong>'):'')+'</span></div>';

 // Meal input sections
 html+='<div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:14px">';
 mealSlots.forEach(function(meal){
  var slotId=meal.replace(/\s/g,'-');
  html+='<div class="card" style="padding:14px">'
   +'<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px">'
   +'<div style="font-size:12.5px;font-weight:700;color:var(--n8)">'+meal+'</div>'
   +'<button class="btn btn-ghost btn-sm" style="font-size:10px" onclick="addRecItem(\''+meal+'\')">+ Adicionar</button></div>'
   +'<div id="rec-items-'+slotId+'"></div>'
   +'</div>';
 });
 html+='</div>';
 ...
 html+='<div class="card" style="padding:14px;margin-bottom:14px">'
  +'<div style="font-size:12.5px;font-weight:700;color:var(--n8);margin-bottom:8px">Observações gerais</div>'
  +'<textarea id="rec-obs-geral" class="sel" style="width:100%;height:60px;resize:vertical;font-size:12px" placeholder="Hidratação, comportamento alimentar, sintomas..."></textarea>'
  +'<div style="display:flex;gap:8px;margin-top:10px;justify-content:flex-end">'
  +'<button class="btn btn-ghost btn-sm" onclick="goP(\'rec\',document.getElementById(\'ni-rec\'))">🔄 Limpar</button>'
  +'<button class="btn btn-p" onclick="saveRec()">💾 Salvar Recordatório</button>'
  +'</div></div>';

 // Previous records
 if(prevRecs.length){
  html+='<div class="card"><div class="ch"><span class="ct">Histórico de Recordatórios</span></div>';
  prevRecs.forEach(function(r){
   var adPct=tkPresc>0?Math.round(Math.min(130,r.totalKcal/tkPresc*100)):0;
   var adColor=adPct>=80?'var(--green)':adPct>=60?'#f59e0b':'var(--red)';
   html+='<div style="display:flex;align-items:center;gap:12px;padding:10px 0;border-bottom:1px solid var(--n1)">'
    +'<div style="font-size:11px;color:var(--n5);min-width:80px">'+r.date+'</div>'
    +'<div style="flex:1">'
    +'<div style="font-size:12px;font-weight:700;color:var(--n8)">~'+r.totalKcal+' kcal consumidas</div>'
    +(r.obs?'<div style="font-size:10.5px;color:var(--n4);margin-top:2px">'+escHtml(r.obs)+'</div>':'')
    +'</div>'
    +(tkPresc?'<div style="font-size:12px;font-weight:800;color:'+adColor+'">'+adPct+'%</div>':'')
    +'</div>';
  });
  html+='</div>';
 }
...
function genAI(){
 ...
 fetch('https://api.anthropic.com/v1/messages',{
  ...
  body:JSON.stringify({
   ...
   max_tokens:2000,
   messages:[{role:'user',content:'Você é um nutricionista. Crie plano alimentar personalizado em JSON puro (sem markdown).\n'
    +'Paciente: '+p.n+', '+p.age+' anos, sexo '+p.sex+', '+p.w+'kg, '+p.h+'cm.\n'
    +'Objetivo: '+objKey+'. Meta: '+Math.round(targetKcal)+' kcal. Refeições: '+nm+'. Restrições: '+(rest||'nenhuma')+'.\n'
    +'Formato EXATO (JSON válido):\n'
    +'{"meals":[{"name":"Café da manhã","em":"☀️","items":[{"food":"Aveia em flocos","qty":50,"kcal":190,"prot":6.5,"carb":33,"gord":3.5}]}],"obs":"orientação geral"}\n'
    +'Use alimentos brasileiros reais. Retorne apenas JSON, sem explicações.'}]
  })
 }).then(function(r){return r.json();}).then(function(data){
  clearInterval(stInterval);
  var text=(data.content&&data.content[0]&&data.content[0].text)||'';
  var parsed=null;
  try{var jm=text.match(/\{[\s\S]*\}/);if(jm)parsed=JSON.parse(jm[0]);}catch(ex){}
  if(!parsed||!parsed.meals){_genAILocal(el,p,objKey,rest,nm,dist,wantSubs,wantLista,wantOri,targetKcal);return;}
  window._aiGenPlan=parsed.meals;
  var tK=0,tP=0,tC=0,tG=0;
  parsed.meals.forEach(function(m){m.items.forEach(function(it){tK+=it.kcal||0;tP+=it.prot||0;tC+=it.carb||0;tG+=it.gord||0;});});
  var html='<div style="display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin-bottom:16px">';
  [['🔥',Math.round(tK)+' kcal','Energia','#e85a0a'],['🥩',tP.toFixed(0)+'g','Proteína','#3b82f6'],['🌾',tC.toFixed(0)+'g','Carboidratos','#f59e0b'],['🥑',tG.toFixed(0)+'g','Gorduras','#10b981']].forEach(function(m){
   html+='<div style="background:#fff;border:1.5px solid var(--n2);border-radius:12px;padding:12px;text-align:center"><div style="font-size:20px">'+m[0]+'</div><div style="font-family:var(--in);font-size:18px;font-weight:800;color:'+m[3]+'">'+m[1]+'</div><div style="font-size:10px;color:var(--n4)">'+m[2]+'</div></div>';
  });
  html+='</div><div style="background:#f0fdf4;border:1.5px solid #bbf7d0;border-radius:12px;padding:10px 14px;margin-bottom:14px;font-size:12px;color:#166534">🤖 Gerado com IA (Claude AI) — revise antes de aplicar</div>';
  parsed.meals.forEach(function(m){
   var mK=m.items.reduce(function(s,it){return s+(it.kcal||0);},0);
   html+='<div class="card" style="margin-bottom:10px"><div class="ch"><span class="ct">'+(m.em||'🍽️')+' '+escHtml(m.name)+'</span><span class="cs">'+mK+' kcal</span></div>';
   m.items.forEach(function(it){html+='<div class="fi"><span class="fi-nm">'+escHtml(it.food)+'</span><span style="font-size:11px;color:var(--n4)">'+it.qty+'g · P:'+it.prot+'g C:'+it.carb+'g G:'+it.gord+'g</span><span class="fi-kcal">'+it.kcal+' kcal</span></div>';});
   html+='</div>';
  });
  if(parsed.obs)html+='<div class="alert alert-g"><span>💡 '+escHtml(parsed.obs)+'</span></div>';
  html+='<div style="display:flex;gap:8px;margin-top:12px"><button class="btn btn-p" style="flex:1" onclick="applyAIGenPlan()">✅ Aplicar este plano</button><button class="btn btn-ghost" onclick="genAI()">🔄 Gerar novamente</button></div>';
  el.innerHTML=html;
  showToast('Plano gerado com IA para '+p.n+'!','s');
 }).catch(function(){
  clearInterval(stInterval);
  _genAILocal(el,p,objKey,rest,nm,dist,wantSubs,wantLista,wantOri,targetKcal);
 });
...
function rNotif(){
 ...
 var all=[].concat(autoAlerts).concat(manualNotifs).sort(function(a,b){
  return (b.ts||b.t||0)-(a.ts||a.t||0);
 });
 ...
 var html='<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:14px;flex-wrap:wrap;gap:8px">'
  +'<div style="display:flex;gap:6px">'
  +'<button class="btn btn-ghost btn-sm" id="nf-all" onclick="_filterNotifs(\'all\',this)" style="font-weight:700">Todas</button>'
  +'<button class="btn btn-ghost btn-sm" id="nf-unread" onclick="_filterNotifs(\'unread\',this)">Não lidas'+(unread?'<span style="background:#ef4444;color:#fff;border-radius:99px;font-size:9px;padding:0 5px;margin-left:4px">'+unread+'</span>':'')+'</button>'
  +'<button class="btn btn-ghost btn-sm" id="nf-auto" onclick="_filterNotifs(\'auto\',this)">Automáticas</button>'
  +'</div>'
  +(unread?'<button class="btn btn-ghost btn-sm" onclick="_markAllNotifsRead()">✓ Marcar todas como lidas</button>':'')
  +'</div>';

 if(!all.length){
  html+='<div class="card" style="text-align:center;padding:48px 20px">'
   +'<div style="font-size:48px;margin-bottom:12px">🔔</div>'
   +'<div style="font-weight:800;font-size:15px;color:var(--n8);margin-bottom:6px">Nenhuma notificação</div>'
   +'<div style="font-size:12px;color:var(--n4)">Você está em dia com todos os pacientes!</div></div>';
  return html;
 }

 html+='<div id="notif-list">';
 all.forEach(function(n,i){
  var typeColors={tg:'#22c55e',ty:'#f59e0b',tr:'#ef4444',tb:'#3b82f6'};
  var typeIcons={tg:'✅',ty:'⚠️',tr:'🔴',tb:'💬'};
  var color=typeColors[n.tag||n.type]||'#6b7280';
  var icon=typeIcons[n.tag||n.type]||'🔔';
  var isRead=!!n.read;
  html+='<div class="notif-item" data-read="'+(isRead?'1':'0')+'" data-auto="'+(n.auto?'1':'0')+'" '
   +'style="display:flex;align-items:flex-start;gap:12px;padding:12px 14px;border-radius:12px;margin-bottom:8px;'
   +'border:1.5px solid '+(isRead?'var(--n1)':'var(--n2)')+';background:'+(isRead?'var(--n0)':'var(--g0)')+';'
   +'border-left:4px solid '+color+';transition:all .15s;cursor:pointer" '
   +'onclick="_markNotifRead('+i+',this)">'
   +'<div style="font-size:20px;flex-shrink:0">'+icon+'</div>'
   +'<div style="flex:1;min-width:0">'
   +'<div style="font-size:12.5px;font-weight:'+(isRead?'500':'700')+';color:var(--n8);margin-bottom:3px">'+escHtml(n.msg||n.text||'')+'</div>'
   +(n.pat?'<div style="font-size:11px;color:var(--n5);margin-bottom:3px">👤 '+escHtml(n.pat)+'</div>':'')
   +'<div style="font-size:10.5px;color:var(--n4)">'+(n.time||n.t||'')+'</div>'
   +'</div>'
   +(!isRead?'<div style="width:8px;height:8px;border-radius:50%;background:#ef4444;flex-shrink:0;margin-top:4px"></div>':'')
   +'</div>';
 });
 html+='</div>';
 ...
 html+='<div class="card" style="margin-top:16px">'
  +'<div class="ch"><span class="ct">Criar lembrete manual</span></div>'
  +'<div style="display:flex;flex-direction:column;gap:10px">'
  +'<select class="sel" id="notif-pat-sel"><option value="">Selecionar paciente...</option>'
  +pats.map(function(p){return'<option value="'+p.id+'">'+escHtml(p.n)+'</option>';}).join('')
  +'</select>'
  +'<input class="inp" id="notif-msg-inp" placeholder="Mensagem do lembrete..."/>'
  +'<button class="btn btn-p" onclick="_addManualNotif()">+ Adicionar lembrete</button>'
  +'</div></div>';
...
function rPDash(){
 ...
 // ── Progresso: meta de peso + adesão ao plano
 if(p.w || p.goalW || p.plans){
  var iniW=hist.length?hist[0].peso||p.w:p.w;
  var curW=lastW||p.w;
  var goalW=p.goalW||p.metaKg||0;
  var plan=p.plans&&p.plans[0]?p.plans[0]:null;
  var kcalMeta=plan?plan.kcal||0:0;

  // Diário alimentar hoje
  var diaryToday=(function(){
   var iso=new Date().toISOString().slice(0,10);
   var ent=(p.diary||[]).find(function(e){return(e.isoDate||'')===iso;});
   return ent?{kcal:ent.totalKcal||0,meals:(ent.meals||[]).length}:{kcal:0,meals:0};
  })();

  html+='<div class="card" style="margin-bottom:14px">';
  html+='<div class="ch" style="margin-bottom:14px"><span class="ct">📈 Progresso</span></div>';

  // Peso: progress bar inicial → atual → meta
  if(goalW&&iniW){
   var total=Math.abs(iniW-goalW);
   var done=Math.abs(iniW-curW);
   var pct=total>0?Math.min(100,Math.round(done/total*100)):0;
   var losing=goalW<iniW;
   html+='<div style="margin-bottom:14px">'
    +'<div style="display:flex;justify-content:space-between;font-size:11px;color:var(--n5);margin-bottom:5px">'
    +'<span>🏁 Meta de peso</span>'
    +'<span><strong>'+pct+'%</strong> concluído</span></div>'
    +'<div style="height:12px;background:var(--n1);border-radius:99px;overflow:hidden">'
    +'<div style="width:'+pct+'%;height:100%;background:var(--g4);border-radius:99px;transition:width .8s ease"></div></div>'
    +'<div style="display:flex;justify-content:space-between;font-size:10.5px;color:var(--n4);margin-top:4px">'
    +'<span>Início: '+iniW.toFixed(1)+'kg</span>'
    +'<span>Atual: <strong style="color:var(--n8)">'+curW.toFixed(1)+'kg</strong></span>'
    +'<span>Meta: '+goalW+'kg</span></div></div>';
  }

  // Calorias hoje vs meta
  if(kcalMeta&&diaryToday.kcal>0){
   var kcalPct=Math.min(100,Math.round(diaryToday.kcal/kcalMeta*100));
   html+='<div style="margin-bottom:14px">'
    +'<div style="display:flex;justify-content:space-between;font-size:11px;color:var(--n5);margin-bottom:5px">'
    +'<span>🍽️ Calorias hoje ('+diaryToday.meals+' refeições)</span>'
    +'<span><strong>'+diaryToday.kcal+'</strong> / '+kcalMeta+' kcal</span></div>'
    +'<div style="height:10px;background:var(--n1);border-radius:99px;overflow:hidden">'
    +'<div style="width:'+kcalPct+'%;height:100%;background:'+(kcalPct>100?'#ef4444':kcalPct>80?'#f59e0b':'#22c55e')+';border-radius:99px"></div></div>'
    +'</div>';
  } else if(kcalMeta){
   html+='<div style="margin-bottom:14px;padding:10px 14px;background:var(--n1);border-radius:10px;font-size:11.5px;color:var(--n4)">'
    +'🍽️ Nenhuma refeição registrada hoje · Meta: '+kcalMeta+' kcal '
    +'<span style="color:var(--g4);cursor:pointer;font-weight:600" onclick="goP(\'pdiary\',null)">→ Registrar agora</span></div>';
  }

  // Adesão ao plano
  var diaryCnt=(p.diary||[]).filter(function(d){
   var dt=new Date(d.isoDate||'');
   var diff=(new Date()-dt)/864e5;
   return diff<=30&&(d.meals&&d.meals.length>0);
  }).length;
  var adherencePct=Math.min(100,Math.round(diaryCnt/30*100));
  html+='<div>'
   +'<div style="display:flex;justify-content:space-between;font-size:11px;color:var(--n5);margin-bottom:5px">'
   +'<span>📋 Adesão ao diário (últimos 30 dias)</span>'
   +'<span><strong>'+diaryCnt+'</strong> / 30 dias</span></div>'
   +'<div style="height:10px;background:var(--n1);border-radius:99px;overflow:hidden">'
   +'<div style="width:'+adherencePct+'%;height:100%;background:'+(adherencePct>=80?'#22c55e':adherencePct>=50?'#f59e0b':'#ef4444')+';border-radius:99px"></div></div>'
   +'</div>';
...
function rPDiary(){
 ...
 var planKcal=(p.plans&&p.plans[0]&&p.plans[0].kcal)?parseInt(p.plans[0].kcal):0;
 var todayEntry=diary.find(function(e){return(e.isoDate||'')===todayISO;});
 ...
 // ── Cabeçalho do dia
 html+='<div class="card" style="margin-bottom:12px">'
  +'<div class="ch"><span class="ct">📋 Diário Alimentar</span>'
  +'<button class="btn btn-p btn-sm" onclick="_openAddMeal()">+ Refeição</button></div>'
  +'<div style="font-size:12px;color:var(--n5);margin-bottom:'+(planKcal?'12px':'4px')+'">'+today.toLocaleDateString("pt-BR",{weekday:"long",day:"2-digit",month:"long"})+'</div>';

 // Barra de progresso calórico
 if(planKcal){
  html+='<div style="margin-bottom:12px">'
   +'<div style="display:flex;justify-content:space-between;font-size:11px;color:var(--n5);margin-bottom:5px">'
   +'<span><strong style="font-size:14px;color:'+(pct>100?'#ef4444':'var(--n9)')+'">'+todayKcal+'</strong> kcal consumidas</span>'
   +'<span>Meta: <strong>'+planKcal+'</strong> kcal</span></div>'
   +'<div style="height:12px;background:var(--n1);border-radius:99px;overflow:hidden">'
   +'<div style="width:'+pct+'%;height:100%;background:'+(pct>100?'#ef4444':pct>80?'#f59e0b':'#22c55e')+';border-radius:99px;transition:width .6s ease"></div></div>'
   +'<div style="font-size:11px;margin-top:4px;text-align:right;color:'+(remaining>=0?'var(--n4)':'#ef4444')+'">'
   +(remaining>0?'Faltam <strong>'+remaining+'</strong> kcal para a meta'
    :remaining===0?'✅ Meta atingida!'
    :'⚠️ Acima da meta em <strong>'+Math.abs(remaining)+'</strong> kcal')+'</div>'
   +'</div>';
 }

 // Refeições do dia
 var meals=todayEntry&&todayEntry.meals?todayEntry.meals:[];
 if(meals.length){
  meals.forEach(function(m,i){
   html+='<div style="display:flex;align-items:flex-start;gap:10px;padding:10px 0;border-top:1px solid var(--n1)">'
    +'<div style="font-size:22px;padding-top:1px">'+(m.emoji||'🍽️')+'</div>'
    +'<div style="flex:1;min-width:0">'
    +'<div style="font-size:12.5px;font-weight:700;color:var(--n8)">'+escHtml(m.name)+'</div>'
    +(m.foods&&m.foods.length?'<div style="font-size:11px;color:var(--n5);margin-top:2px">'+m.foods.map(function(f){return escHtml(f);}).join(' · ')+'</div>':'')
    +(m.obs?'<div style="font-size:10.5px;color:var(--n4);margin-top:2px;font-style:italic">'+escHtml(m.obs)+'</div>':'')
    +'</div>'
    +'<div style="display:flex;align-items:center;gap:8px;flex-shrink:0">'
    +'<span style="font-family:var(--in);font-size:13px;font-weight:700;color:var(--g4)">'+m.kcal+' kcal</span>'
    +'<button onclick="_removeDiaryMeal(\''+todayISO+'\','+i+')" style="background:none;border:none;cursor:pointer;color:var(--n3);font-size:16px;padding:2px 4px;line-height:1" title="Remover">✕</button>'
    +'</div></div>';
  });
 } else {
  html+='<div style="text-align:center;padding:20px 0 8px;color:var(--n3);font-size:12px">Nenhuma refeição registrada hoje.<br>'
   +'<span style="color:var(--g4);cursor:pointer;font-weight:600" onclick="_openAddMeal()">+ Registrar agora</span></div>';
 }
 html+='</div>';
 ...
 for(var i=6;i>=0;i--){
  ...
  var ent=diary.find(function(e){return(e.isoDate||'')===iso;});
  diary7.push({iso:iso,day:d,kcal:ent?(ent.totalKcal||0):0,meals:ent?(ent.meals||[]).length:0});
 }
 var maxK=Math.max.apply(null,diary7.map(function(d){return d.kcal;}));
 maxK=Math.max(maxK,planKcal||2000,500);
 var wAvg=Math.round(diary7.reduce(function(s,d){return s+d.kcal;},0)/diary7.filter(function(d){return d.kcal>0;}).length)||0;

 html+='<div class="card" style="margin-bottom:12px"><div class="ch" style="margin-bottom:10px">'
  +'<span class="ct">Últimos 7 dias</span>'
  +'<span style="font-size:11px;color:var(--n4)">Média: <strong style="color:var(--n8)">'+wAvg+'</strong> kcal</span></div>'
  +'<div style="display:flex;align-items:flex-end;gap:5px;height:90px;padding:0 4px">';
 diary7.forEach(function(d){
  var h=d.kcal?Math.max(6,Math.round(d.kcal/maxK*82)):3;
  var isToday=d.iso===todayISO;
  var overGoal=planKcal&&d.kcal>planKcal;
  var color=overGoal?'#ef4444':(d.kcal>0?(isToday?'var(--g4)':'var(--g3)'):'var(--n2)');
  html+='<div style="flex:1;display:flex;flex-direction:column;align-items:center;gap:3px">'
   +'<div style="font-size:8.5px;color:var(--n4);line-height:1">'+(d.kcal||'')+'</div>'
   +'<div style="width:100%;height:'+h+'px;background:'+color+';border-radius:4px 4px 0 0;'+(isToday?'box-shadow:0 0 0 2px var(--g4)':'')+'" title="'+d.kcal+' kcal"></div>'
   +'<div style="font-size:9px;color:'+(isToday?'var(--g4)':'var(--n5)')+';font-weight:'+(isToday?'700':'400')+'">'+dNames[d.day.getDay()]+'</div>'
   +'</div>';
 });
 if(planKcal){
  // Meta line indicator
  html+='</div><div style="display:flex;align-items:center;gap:6px;margin-top:8px;font-size:10px;color:var(--n4)">'
   +'<div style="width:16px;height:2px;background:#f59e0b;border-radius:2px"></div>Meta '+planKcal+' kcal'
   +'<div style="width:16px;height:2px;background:#ef4444;border-radius:2px;margin-left:8px"></div>Acima da meta</div>';
 }
 html+='</div></div>';

 // ── Histórico
 var past=diary.slice().sort(function(a,b){return(b.isoDate||'')>(a.isoDate||'')?1:-1;}).filter(function(e){return(e.isoDate||'')!==todayISO;}).slice(0,14);
 if(past.length){
  html+='<div class="card"><div class="ch"><span class="ct">Histórico</span></div>';
  past.forEach(function(d){
   var dt=new Date(d.isoDate||'');
   var k=d.totalKcal||0;
   var barW=planKcal?Math.min(100,Math.round(k/planKcal*100)):Math.min(100,Math.round(k/2000*100));
   html+='<div style="display:flex;align-items:center;gap:12px;padding:9px 0;border-bottom:1px solid var(--n1)">'
    +'<div style="min-width:72px"><div style="font-size:12px;font-weight:700;color:var(--n8)">'
    +(isNaN(dt)?d.isoDate:dt.toLocaleDateString('pt-BR',{day:'2-digit',month:'short',weekday:'short'}))+'</div>'
    +(d.meals&&d.meals.length?'<div style="font-size:10px;color:var(--n4)">'+d.meals.length+' refeições</div>':'')
    +'</div>'
    +'<div style="flex:1;height:7px;background:var(--n1);border-radius:99px;overflow:hidden">'
    +'<div style="width:'+barW+'%;height:100%;background:'+(k>planKcal&&planKcal?'#ef4444':'var(--g4)')+';border-radius:99px"></div></div>'
    +'<div style="font-family:var(--in);font-size:12px;font-weight:700;color:var(--n7);min-width:55px;text-align:right">'+k+' kcal</div>'
    +'</div>';
  });
  html+='</div>';
 }
...
function _openAddMeal(){
 var mo=document.getElementById('m-generic');
 if(!mo){showToast('Erro ao abrir formulário','e');return;}
 ...
 mo.querySelector('.m-body').innerHTML=
  ...
  +'<input id="meal-kcal" class="inp" type="number" placeholder="ex: 450" min="0" style="width:100%"/></div>'
  +'</div>'
  +'<div><label class="lbl">Alimentos (separados por vírgula)</label>'
  +'<input id="meal-foods" class="inp" placeholder="ex: Arroz, Feijão, Frango grelhado" style="width:100%"/></div>'
  +'<div><label class="lbl">Proteína (g)</label>'
  +'<input id="meal-prot" class="inp" type="number" placeholder="ex: 35" min="0" style="width:100%"/></div>'
  +'<div><label class="lbl">Observação</label>'
  +'<input id="meal-obs" class="inp" placeholder="Como se sentiu? Teve fome?" style="width:100%"/></div>'
  +'<button class="btn btn-p" onclick="_saveDiaryMeal()" style="width:100%">💾 Salvar refeição</button>'
  +'</div>';
 openM('m-generic');
...
function _saveDiaryMeal(){
 ...
 var protEl=document.getElementById('meal-prot');
 var obsEl=document.getElementById('meal-obs');
 if(!typeEl||!kcalEl)return;
 var full=typeEl.value;
 ...
 var prot=protEl?parseInt(protEl.value)||0:0;
 var obs=obsEl?obsEl.value:'';
 var todayISO=new Date().toISOString().slice(0,10);
 if(!p.diary)p.diary=[];
 var entry=p.diary.find(function(e){return(e.isoDate||'')===todayISO;});
 if(!entry){entry={isoDate:todayISO,meals:[],totalKcal:0,totalProt:0};p.diary.push(entry);}
 if(!entry.meals)entry.meals=[];
 var now=new Date();
 entry.meals.push({
  emoji:emoji,name:name,kcal:kcal,prot:prot,foods:foods,obs:obs,
  time:now.toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'})
 });
 entry.totalKcal=(entry.totalKcal||0)+kcal;
 entry.totalProt=(entry.totalProt||0)+prot;
 DB.save();
 closeM('m-generic');
...
function _removeDiaryMeal(isoDate,idx){
 var p=_getPacPatient();if(!p||!p.diary)return;
 var entry=p.diary.find(function(e){return(e.isoDate||'')===isoDate;});
 if(!entry||!entry.meals||!entry.meals[idx])return;
 entry.totalKcal=Math.max(0,(entry.totalKcal||0)-(entry.meals[idx].kcal||0));
 entry.totalProt=Math.max(0,(entry.totalProt||0)-(entry.meals[idx].prot||0));
 entry.meals.splice(idx,1);
 DB.save();
 showToast('Refeição removida','s');
 goP('pdiary',document.getElementById('ni-pdiary'));
}
...
function openChat(patId){
 ...
 if(!p.msgs)p.msgs=[];
 var title=document.getElementById('chat-title');
 if(title)title.textContent='Chat — '+p.n;
 _renderChat(p);
 openM('m-chat');
 setTimeout(function(){
  var inp=document.getElementById('chat-input');
...
// ════════════════════════════════════════════════════════════
function rChat(){
 // List all patients that have messages, sorted by most recent
 var chatPats = pats.map(function(p){
  var msgs = p.msgs||[];
  var last = msgs.length ? msgs[msgs.length-1] : null;
  var unread = msgs.filter(function(m){return m.role==='pac'&&!m.read;}).length;
  return {p:p, last:last, unread:unread};
 }).filter(function(x){return x.last;})
   .sort(function(a,b){return (b.last.ts||'') > (a.last.ts||'') ? 1 : -1;});
 ...
 html += '<div style="display:flex;align-items:center;gap:8px;margin-bottom:14px">'
  ...
  +'<input class="search-inp" id="chat-search" placeholder="Buscar conversa…" oninput="_filterChatList(this.value)"/></div>'
  +'</div>';

 if(!chatPats.length){
  html += '<div class="card" style="text-align:center;padding:48px 20px">'
   +'<div style="font-size:48px;margin-bottom:12px">💬</div>'
   +'<div style="font-weight:800;font-size:15px;color:var(--n8);margin-bottom:8px">Nenhuma conversa ainda</div>'
   +'<div style="font-size:12px;color:var(--n4);margin-bottom:16px">Inicie uma conversa a partir da aba de Pacientes</div>'
   +'<button class="btn btn-p" onclick="goP(\'pat\',document.getElementById(\'ni-pat\'))">Ver Pacientes</button>'
   +'</div>';
  return html;
 }

 html += '<div id="chat-list">';
 chatPats.forEach(function(x){
  var p = x.p, last = x.last, unread = x.unread;
  var lastText = last ? (last.role==='nutri'?'Você: ':'')+last.txt : '';
  if(lastText.length > 52) lastText = lastText.slice(0,52)+'…';
  html += '<div class="card" style="display:flex;align-items:center;gap:12px;padding:12px 14px;cursor:pointer;margin-bottom:8px;transition:border-color .15s" '
   +'onclick="openChat('+p.id+')" onmouseover="this.style.borderColor=\'var(--g4)\'" onmouseout="this.style.borderColor=\'\'">'
   +'<div class="av '+(p.av||'a1')+'" style="width:44px;height:44px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:16px;font-weight:800;flex-shrink:0">'+escHtml(p.i||p.n[0])+'</div>'
   +'<div style="flex:1;min-width:0">'
   +'<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:2px">'
   +'<span style="font-weight:700;font-size:13px;color:var(--n9)">'+escHtml(p.n)+'</span>'
   +'<span style="font-size:10px;color:var(--n4)">'+(last?last.time:'')+'</span>'
   +'</div>'
   +'<div style="display:flex;justify-content:space-between;align-items:center">'
   +'<span style="font-size:11.5px;color:'+(unread?'var(--n8)':'var(--n4)')+';font-weight:'+(unread?'600':'400')+';overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:220px">'+escHtml(lastText)+'</span>'
   +(unread?'<span style="background:var(--g4);color:#fff;border-radius:99px;font-size:10px;font-weight:700;padding:1px 7px;flex-shrink:0">'+unread+'</span>':'')
   +'</div></div></div>';
 });
 html += '</div>';

 // Also show patients WITHOUT messages (so nutri can start a chat)
 var noChatPats = pats.filter(function(p){return !(p.msgs&&p.msgs.length);});
 if(noChatPats.length){
  html += '<div style="margin-top:16px"><div style="font-size:11px;font-weight:700;color:var(--n4);text-transform:uppercase;letter-spacing:.05em;margin-bottom:8px">Iniciar nova conversa</div>';
  noChatPats.forEach(function(p){
   html += '<div style="display:flex;align-items:center;gap:10px;padding:8px 12px;border-radius:10px;cursor:pointer;transition:background .15s" '
    +'onclick="openChat('+p.id+')" onmouseover="this.style.background=\'var(--n1)\'" onmouseout="this.style.background=\'\'">'
    +'<div class="av '+(p.av||'a1')+'" style="width:34px;height:34px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:13px;font-weight:800;flex-shrink:0">'+escHtml(p.i||p.n[0])+'</div>'
    +'<span style="font-size:12.5px;font-weight:600;color:var(--n7)">'+escHtml(p.n)+'</span>'
    +'<span style="margin-left:auto;font-size:11px;color:var(--g4)">+ Iniciar conversa →</span>'
    +'</div>';
  });
  html += '</div>';
 }
